            ...
            if (ImGui::BeginTabItem("Global"))
            {
                if (_state->_currentTrack >= 0)
                {
                    ...
                    for (unsigned char i = SAMPLE_NOTE_MAX; i >= SAMPLE_NOTE_MIN; i--)
                    {
                        ...
                        {
                            ImGui::Text("%s", parameters->PwavData->name.c_str());
                        }
                        else
                        {
                            ImGui::Text("<unused>");
                        ...
                        if (parameters->PwavData != nullptr)
                        {
                            ...
                            {
                                delete parameters->PwavData;
                                parameters->PwavData = nullptr;
                            }
                        }
                        ImGui::NextColumn();
                        ImGui::PopID();
                    ...
                    if (ImGui::BeginPopupModal("Select sample", nullptr, ImGuiWindowFlags_NoResize))
                    {
                        ...
                        if (ImGui::Button("Ok", ImVec2(120, 0)))
                        {
                            ...
                            {
                                parameters->PwavData = WavData::Load(_library.GetSelectedSample()->GetPath());
                            }
