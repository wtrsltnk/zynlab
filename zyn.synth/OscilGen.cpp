...
void OscilGen::getbasefunction(float *smps)
{
    ...
    switch (Pbasefuncmodulation)
    {
        ...
        case 3:
            ...
            basefuncmodulationpar3 = 0.01f + (powf(2, basefuncmodulationpar3 * 16.0f) - 1.0f) / 10.0f;
            break;
    }

    base_func func = getBaseFunction(Pcurrentbasefunc);

    for (i = 0; i < this->_synth->oscilsize; ++i)
    {
        ...
        switch (Pbasefuncmodulation)
        {
            case 1:
                t = t * basefuncmodulationpar3 + sinf(
                                                     (t + basefuncmodulationpar2) * 2.0f * PI) *
                                                     basefuncmodulationpar1; //rev
                break;
            case 2:
                t = t + sinf(
                            (t * basefuncmodulationpar3 + basefuncmodulationpar2) * 2.0f * PI) *
                            basefuncmodulationpar1; //sine
                break;
            case 3:
                t = t + powf((1.0f - cosf(
                                         (t + basefuncmodulationpar2) * 2.0f * PI)) *
...
void OscilGen::waveshape()
{
    ...
    for (int i = 1; i < this->_synth->oscilsize / 8; ++i)
    {
        float gain = i / (this->_synth->oscilsize / 8.0f);
...
void OscilGen::modulation()
{
    ...
    //reduce the amplitude of the freqs near the nyquist
    for (i = 1; i < this->_synth->oscilsize / 8; ++i)
    {
        float tmp = i / (this->_synth->oscilsize / 8.0f);
    ...
    for (i = 0; i < this->_synth->oscilsize; ++i)
    {
        ...
        {
            case 1:
                t = t * modulationpar3 + sinf((t + modulationpar2) * 2.0f * PI) * modulationpar1; //rev
                break;
            case 2:
                t = t + sinf((t * modulationpar3 + modulationpar2) * 2.0f * PI) * modulationpar1; //sine
                break;
            case 3:
                t = t + powf((1.0f - cosf(
                                         (t + modulationpar2) * 2.0f * PI)) *
                                 0.5f,
                             modulationpar3) *
                            modulationpar1; //power
                break;
        }
...
void OscilGen::shiftharmonics()
{
    ...
    if (harmonicshift > 0)
        ...
        for (int i = 0; i < this->_synth->oscilsize / 2 - 1; ++i)
        {
...
{
    if ((oldbasepar != Pbasefuncpar) || (oldbasefunc != Pcurrentbasefunc) || DIFF(basefuncmodulation) || DIFF(basefuncmodulationpar1) || DIFF(basefuncmodulationpar2) || DIFF(basefuncmodulationpar3))
        changebasefunction();
...
bool OscilGen::needPrepare()
{
    ...
    if ((oldbasepar != Pbasefuncpar) || (oldbasefunc != Pcurrentbasefunc) || DIFF(hmagtype) || DIFF(waveshaping) || DIFF(waveshapingfunction))
        outdated = true;
    ...
    if (DIFF(basefuncmodulation) || DIFF(basefuncmodulationpar1) || DIFF(basefuncmodulationpar2) || DIFF(basefuncmodulationpar3))
        outdated = true;

    //Check overall modulation
    if (DIFF(modulation) || DIFF(modulationpar1) || DIFF(modulationpar2) || DIFF(modulationpar3))
        outdated = true;
...
short int OscilGen::get(float *smps, float freqHz, int resonance)
{
    ...
    if (Padaptiveharmonics) //do the antialiasing in the case of adaptive harmonics
        for (int i = nyquist; i < this->_synth->oscilsize / 2; ++i)
