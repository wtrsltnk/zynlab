...
{
    tmpwavel = new float[SystemSettings::Instance().buffersize];
    tmpwaver = new float[SystemSettings::Instance().buffersize];
    bypassl = new float[SystemSettings::Instance().buffersize];
    bypassr = new float[SystemSettings::Instance().buffersize];
    ...
    if (pars->PPunchStrength != 0)
    {
        ...
        NoteGlobalPar.Punch.initialvalue = ((powf(10, 1.5f * pars->PPunchStrength / 127.0f) - 1.0f) * VelF(velocity, pars->PPunchVelocitySensing));
        float time = powf(10, 3.0f * pars->PPunchTime / 127.0f) / 10000.0f; //0.1f .. 100 ms
        float stretch = powf(440.0f / freq,
                             pars->PPunchStretch / 64.0f);
        NoteGlobalPar.Punch.dt = 1.0f / (time * SystemSettings::Instance().samplerate_f * stretch);
    }
    else
    ...
    for (int nvoice = 0; nvoice < NUM_VOICES; ++nvoice)
    {
        ...
        NoteVoicePar[nvoice].OscilSmp = nullptr;
        NoteVoicePar[nvoice].FMSmp = nullptr;
        NoteVoicePar[nvoice].VoiceOut = nullptr;
        ...
        float unison_vibratto_a = pars->VoicePar[nvoice].Unison_vibratto / 127.0f; //0.0f .. 1.0f
        ...
        switch (unison)
        {
            case 1:
                ...
                break;
            case 2:
            { //unison for 2 subvoices
                unison_base_freq_rap[nvoice][0] = 1.0f / unison_real_spread;
                unison_base_freq_rap[nvoice][1] = unison_real_spread;
            };
            break;
            default:
            { //unison for more than 2 subvoices
                std::unique_ptr<float> unison_values(new float[unison]);
                float min = -1e-6f, max = 1e-6f;
                for (int k = 0; k < unison; ++k)
                {
                    float step = (k / static_cast<float>(unison - 1)) * 2.0f - 1.0f; //this makes the unison spread more uniform
                    float val = step + (RND * 2.0f - 1.0f) / (unison - 1);
                    unison_values.get()[k] = val;
                    if (min > val)
                    {
                        min = val;
                    }
                ...
                {
                    unison_values.get()[k] =
                        (unison_values.get()[k] - (max + min) * 0.5f) / diff; //the lowest value will be -1 and the highest will be 1
                    unison_base_freq_rap[nvoice][k] =
                        powf(2.0f, (unison_spread * unison_values.get()[k]) / 1200);
                }
            };
        }
        ...
        float increments_per_second = SystemSettings::Instance().samplerate_f / SystemSettings::Instance().buffersize_f;
        float vibratto_base_period = 0.25f * powf(
                                                 2.0f,
                                                 (1.0f - pars->VoicePar[nvoice].Unison_vibratto_speed / 127.0f) * 4.0f);
        for (int k = 0; k < unison; ++k)
        {
            unison_vibratto[nvoice].position[k] = RND * 1.8f - 0.9f;
            //make period to vary randomly from 50% to 200% vibratto base period
            float vibratto_period = vibratto_base_period * powf(2.0f, RND * 2.0f - 1.0f);
        ...
        if (unison == 1)
        { //no vibratto for a single voice
        ...
        if (unison != 1)
        {
            ...
            switch (inv)
            {
                ...
                default:
                    for (int k = 0; k < unison; ++k)
                        unison_invert_phase[nvoice][k] = (k % inv == 0);
                    break;
            }
        }
        ...
        NoteVoicePar[nvoice].OscilSmp =
            new float[SystemSettings::Instance().oscilsize + OSCIL_SMP_EXTRA_SAMPLES];
        ...
        for (int i = 0; i < OSCIL_SMP_EXTRA_SAMPLES; ++i)
            NoteVoicePar[nvoice].OscilSmp[SystemSettings::Instance().oscilsize + i] =
                NoteVoicePar[nvoice].OscilSmp[i];

        oscposhi_start +=
            static_cast<int>((pars->VoicePar[nvoice].Poscilphase - 64.0f) / 128.0f * SystemSettings::Instance().oscilsize + SystemSettings::Instance().oscilsize * 4);
        oscposhi_start %= SystemSettings::Instance().oscilsize;
        ...
        for (int k = 0; k < unison; ++k)
        {
            ...
            oscposhi_start =
                static_cast<int>(RND * pars->VoicePar[nvoice].Unison_phase_randomness /
                                 127.0f * (SystemSettings::Instance().oscilsize - 1));
        }

        NoteVoicePar[nvoice].FreqLfo = nullptr;
        NoteVoicePar[nvoice].FreqEnvelope = nullptr;

        NoteVoicePar[nvoice].AmpLfo = nullptr;
        NoteVoicePar[nvoice].AmpEnvelope = nullptr;

        NoteVoicePar[nvoice].VoiceFilterL = nullptr;
        NoteVoicePar[nvoice].VoiceFilterR = nullptr;
        NoteVoicePar[nvoice].FilterEnvelope = nullptr;
        NoteVoicePar[nvoice].FilterLfo = nullptr;
        ...
        NoteVoicePar[nvoice].FMFreqEnvelope = nullptr;
        NoteVoicePar[nvoice].FMAmpEnvelope = nullptr;
        ...
        NoteVoicePar[nvoice].DelayTicks =
            static_cast<int>((expf(pars->VoicePar[nvoice].PDelay / 127.0f * logf(50.0f)) - 1.0f) / SystemSettings::Instance().buffersize_f / 10.0f * SystemSettings::Instance().samplerate_f);
    }

    max_unison = 1;
    for (int nvoice : unison_size)
        if (nvoice > max_unison)
            max_unison = nvoice;

    tmpwave_unison = new float *[max_unison];
    for (int k = 0; k < max_unison; ++k)
    {
        tmpwave_unison[k] = new float[SystemSettings::Instance().buffersize];
        memset(tmpwave_unison[k], 0, SystemSettings::Instance().bufferbytes);
    }
    ...
    bool externcall)
{
    ...
    for (int nvoice = 0; nvoice < NUM_VOICES; ++nvoice)
    {
        ...
        //I store the first elments to the last position for speedups
        for (unsigned int i = 0; i < OSCIL_SMP_EXTRA_SAMPLES; ++i)
            NoteVoicePar[nvoice].OscilSmp[SystemSettings::Instance().oscilsize + i] =
                NoteVoicePar[nvoice].OscilSmp[i];
    ...
    NoteGlobalPar.Volume = 4.0f * powf(0.1f, 3.0f * (1.0f - partparams->PVolume / 96.0f)) //-60 dB .. 0 dB
                           * VelF(
                                 velocity,
    ...
    for (unsigned nvoice = 0; nvoice < NUM_VOICES; ++nvoice)
    {
        ...
        if ((partparams->VoicePar[nvoice].PAmpEnvelopeEnabled != 0) && (NoteVoicePar[nvoice].AmpEnvelope != nullptr))
            newamplitude[nvoice] *= NoteVoicePar[nvoice].AmpEnvelope->envout_dB();

        if ((partparams->VoicePar[nvoice].PAmpLfoEnabled != 0) && (NoteVoicePar[nvoice].AmpLfo != nullptr))
        ...
        if ((NoteVoicePar[nvoice].FMEnabled != NONE) && (NoteVoicePar[nvoice].FMVoice < 0))
        {
            ...
            for (int i = 0; i < OSCIL_SMP_EXTRA_SAMPLES; ++i)
                NoteVoicePar[nvoice].FMSmp[SystemSettings::Instance().oscilsize + i] =
                    NoteVoicePar[nvoice].FMSmp[i];
        }

        FMnewamplitude[nvoice] = NoteVoicePar[nvoice].FMVolume * ctl->fmamp.relamp;

        if ((partparams->VoicePar[nvoice].PFMAmpEnvelopeEnabled != 0) && (NoteVoicePar[nvoice].FMAmpEnvelope != nullptr))
...
void ADnote::initparameters()
{
    ...
    // Global Parameters
    NoteGlobalPar.initparameters(*partparams, basefreq, velocity, stereo);
    ...
    for (int nvoice = 0; nvoice < NUM_VOICES; ++nvoice)
    {
        ...
        if ((vce.FMEnabled != NONE) && (vce.FMVoice < 0))
        {
            ...
            vce.FMSmp = new float[SystemSettings::Instance().oscilsize + OSCIL_SMP_EXTRA_SAMPLES];
            ...
            float tmp = 1.0f;
            if ((partparams->VoicePar[vc].FMSmp->Padaptiveharmonics != 0) || (vce.FMEnabled == MORPH) || (vce.FMEnabled == RING_MOD))
            ...
            for (int k = 0; k < unison_size[nvoice]; ++k)
                oscposhiFM[nvoice][k] = (oscposhi[nvoice][k] + partparams->VoicePar[vc].FMSmp->get(
                                                                   vce.FMSmp, tmp)) %
                                        SystemSettings::Instance().oscilsize;

            for (int i = 0; i < OSCIL_SMP_EXTRA_SAMPLES; ++i)
                vce.FMSmp[SystemSettings::Instance().oscilsize + i] = vce.FMSmp[i];
            auto oscposhiFM_add =
                static_cast<int>((param.PFMoscilphase - 64.0f) / 128.0f * SystemSettings::Instance().oscilsize + SystemSettings::Instance().oscilsize * 4);
            for (int k = 0; k < unison_size[nvoice]; ++k)
            {
                oscposhiFM[nvoice][k] += oscposhiFM_add;
                oscposhiFM[nvoice][k] %= SystemSettings::Instance().oscilsize;
            }
        }
    ...
    for (int nvoice = 0; nvoice < NUM_VOICES; ++nvoice)
    {
        ...
        for (int i = nvoice + 1; i < NUM_VOICES; ++i)
            if ((NoteVoicePar[i].FMVoice == nvoice) && (tmp[i] == 0))
            {
                NoteVoicePar[nvoice].VoiceOut = new float[SystemSettings::Instance().buffersize];
                tmp[i] = 1;
            }

        if (NoteVoicePar[nvoice].VoiceOut)
            memset(NoteVoicePar[nvoice].VoiceOut, 0, SystemSettings::Instance().bufferbytes);
    }
}
 ...
 */
void ADnote::compute_unison_freq_rap(
    int nvoice)
{
    ...
    float in_freq)
{
    for (int k = 0; k < unison_size[nvoice]; ++k)
    {
        float freq = std::fabs(in_freq) * unison_freq_rap[nvoice][k];
        float speed = freq * SystemSettings::Instance().oscilsize_f / SystemSettings::Instance().samplerate_f;
        if (speed > SystemSettings::Instance().oscilsize_f)
            speed = SystemSettings::Instance().oscilsize_f;

        F2I(speed, oscfreqhi[nvoice][k]);
        oscfreqlo[nvoice][k] = speed - std::floor(speed);
    }
}
    ...
    float in_freq)
{
    for (int k = 0; k < unison_size[nvoice]; ++k)
    {
        float freq = std::fabs(in_freq) * unison_freq_rap[nvoice][k];
        float speed = freq * SystemSettings::Instance().oscilsize_f / SystemSettings::Instance().samplerate_f;
        if (speed > SystemSettings::Instance().samplerate_f)
            speed = SystemSettings::Instance().samplerate_f;

        F2I(speed, oscfreqhiFM[nvoice][k]);
        oscfreqloFM[nvoice][k] = speed - std::floor(speed);
    }
}
 ...
 */
float ADnote::getvoicebasefreq(
    int nvoice) const
{
    float detune = NoteVoicePar[nvoice].Detune / 100.0f + NoteVoicePar[nvoice].FineDetune / 100.0f * ctl->bandwidth.relbw * bandwidthDetuneMultiplier + NoteGlobalPar.Detune / 100.0f;
 ...
 */
float ADnote::getFMvoicebasefreq(
    int nvoice) const
{
...
void ADnote::computecurrentparameters()
{
    ...
    for (nvoice = 0; nvoice < NUM_VOICES; ++nvoice)
    {
        ...
        if (NoteVoicePar[nvoice].AmpEnvelope != nullptr)
            newamplitude[nvoice] *= NoteVoicePar[nvoice].AmpEnvelope->envout_dB();

        if (NoteVoicePar[nvoice].AmpLfo != nullptr)
            newamplitude[nvoice] *= NoteVoicePar[nvoice].AmpLfo->amplfoout();

        /****************/
        /* Voice Filter */
        /****************/
        if (NoteVoicePar[nvoice].VoiceFilterL != nullptr)
        {
            filterpitch = NoteVoicePar[nvoice].FilterCenterPitch;

            if (NoteVoicePar[nvoice].FilterEnvelope != nullptr)
                filterpitch += NoteVoicePar[nvoice].FilterEnvelope->envout();

            if (NoteVoicePar[nvoice].FilterLfo != nullptr)
        ...
        { //compute only if the voice isn't noise
            ...
            /*******************/
            voicepitch = 0.0f;
            if (NoteVoicePar[nvoice].FreqLfo != nullptr)
                voicepitch += NoteVoicePar[nvoice].FreqLfo->lfoout() / 100.0f * ctl->bandwidth.relbw;

            if (NoteVoicePar[nvoice].FreqEnvelope != nullptr)
                voicepitch += NoteVoicePar[nvoice].FreqEnvelope->envout() / 100.0f;
            voicefreq = getvoicebasefreq(nvoice) * powf(2, (voicepitch + globalpitch) / 12.0f); //Hz frequency
            ...
            if (NoteVoicePar[nvoice].FMEnabled != NONE)
            {
                ...
                if (NoteVoicePar[nvoice].FMFreqEnvelope != nullptr)
                    FMrelativepitch +=
                        NoteVoicePar[nvoice].FMFreqEnvelope->envout() / 100;
                FMfreq =
                    powf(2.0f, FMrelativepitch / 12.0f) * voicefreq * portamentofreqrap;
                setfreqFM(nvoice, FMfreq);

                FMoldamplitude[nvoice] = FMnewamplitude[nvoice];
                FMnewamplitude[nvoice] = NoteVoicePar[nvoice].FMVolume * ctl->fmamp.relamp;
                if (NoteVoicePar[nvoice].FMAmpEnvelope != nullptr)
                    FMnewamplitude[nvoice] *=
                        NoteVoicePar[nvoice].FMAmpEnvelope->envout_dB();
            }
        }
    }
    time += SystemSettings::Instance().buffersize_f / SystemSettings::Instance().samplerate_f;
}

/*
 * Fadein in a way that removes clicks but keep sound "punchy"
 */
inline void ADnote::fadein(
    float *smps) const
{
    int zerocrossings = 0;
    for (unsigned int i = 1; i < SystemSettings::Instance().buffersize; ++i)
        if ((smps[i - 1] < 0.0f) && (smps[i] > 0.0f))
            zerocrossings++; //this is only the possitive crossings

    float tmp = (SystemSettings::Instance().buffersize_f - 1.0f) / (zerocrossings + 1) / 3.0f;
    if (tmp < 8.0f)
        tmp = 8.0f;
 ...
 */
inline void ADnote::ComputeVoiceOscillator_LinearInterpolation(
    int nvoice)
{
    for (int k = 0; k < unison_size[nvoice]; ++k)
    {
        ...
        for (int i = 0; i < SystemSettings::Instance().buffersize; ++i)
        {
            ...
            poslo &= 0xffffff;
            poshi &= SystemSettings::Instance().oscilsize - 1;
        }
 ...
 */
inline void ADnote::ComputeVoiceOscillatorMorph(
    int nvoice)
{
    float amp;
    ...
    if (NoteVoicePar[nvoice].FMVoice >= 0)
    {
        ...
        for (int k = 0; k < unison_size[nvoice]; ++k)
        {
            unsigned int poshiFM = oscposhiFM[nvoice][k];
            float posloFM = oscposloFM[nvoice][k];
            unsigned int freqhiFM = oscfreqhiFM[nvoice][k];
            ...
            for (unsigned int i = 0; i < SystemSettings::Instance().buffersize; ++i)
            {
                ...
                poshiFM += freqhiFM;
                poshiFM &= SystemSettings::Instance().oscilsize - 1;
            }
 ...
 */
inline void ADnote::ComputeVoiceOscillatorRingModulation(
    int nvoice)
{
    float amp;
    ComputeVoiceOscillator_LinearInterpolation(nvoice);
        ...
        for (int k = 0; k < unison_size[nvoice]; ++k)
        {
            ...
            for (unsigned int i = 0; i < SystemSettings::Instance().buffersize; ++i)
            {
                amp = INTERPOLATE_AMPLITUDE(FMoldamplitude[nvoice],
                                            ...
                                            i,
                                            SystemSettings::Instance().buffersize);
                tw[i] *= (NoteVoicePar[nvoice].FMSmp[poshiFM] * (1.0f - posloFM) + NoteVoicePar[nvoice].FMSmp[poshiFM + 1] * posloFM) * amp + (1.0f - amp);
                posloFM += freqloFM;
                if (posloFM >= 1.0f)
                {
                    posloFM -= 1.0f;
 ...
 */
inline void ADnote::ComputeVoiceOscillatorFrequencyModulation(
    int nvoice,
    int FMmode)
{
    int carposhi = 0;
    ...
    if (NoteVoicePar[nvoice].FMVoice >= 0)
        ...
        for (int k = 0; k < unison_size[nvoice]; ++k)
        {
            ...
            memcpy(tw, NoteVoicePar[NoteVoicePar[nvoice].FMVoice].VoiceOut,
                   SystemSettings::Instance().bufferbytes);
        }
    else
        ...
        for (int k = 0; k < unison_size[nvoice]; ++k)
        {
            ...
            {
                tw[i] =
                    (NoteVoicePar[nvoice].FMSmp[poshiFM] * (1.0f - posloFM) + NoteVoicePar[nvoice].FMSmp[poshiFM + 1] * posloFM);
                posloFM += freqloFM;
                if (posloFM >= 1.0f)
                {
                    posloFM = std::fmod(posloFM, 1.0f);
                    poshiFM++;
                }
                poshiFM += freqhiFM;
                poshiFM &= SystemSettings::Instance().oscilsize - 1;
    ...
    { //Frequency modulation
        float normalize = SystemSettings::Instance().oscilsize_f / 262144.0f * 44100.0f / SystemSettings::Instance().samplerate_f;
        for (int k = 0; k < unison_size[nvoice]; ++k)
        {
            float *tw = tmpwave_unison[k];
            ...
            for (unsigned int i = 0; i < SystemSettings::Instance().buffersize; ++i)
            {
                fmold = fmod(fmold + tw[i] * normalize, SystemSettings::Instance().oscilsize);
                tw[i] = fmold;
            }
    ...
    { //Phase modulation
        float normalize = SystemSettings::Instance().oscilsize_f / 262144.0f;
        for (int k = 0; k < unison_size[nvoice]; ++k)
        {
            float *tw = tmpwave_unison[k];
            for (unsigned int i = 0; i < SystemSettings::Instance().buffersize; ++i)
    ...
    for (int k = 0; k < unison_size[nvoice]; ++k)
    {
        ...
        for (unsigned int i = 0; i < SystemSettings::Instance().buffersize; ++i)
        {
            ...
            FMmodfreqlo = std::fmod(tw[i] + 0.0000000001f, 1.0f);
            if (FMmodfreqhi < 0)
                FMmodfreqlo++;
            ...
            {
                carposhi++;
                carposlo = std::fmod(carposlo, 1.0f);
            }
            carposhi &= (SystemSettings::Instance().oscilsize - 1);
...
inline void ADnote::ComputeVoiceOscillatorPitchModulation(
    int /*nvoice*/)
{
    //TODO
}
 ...
 * Computes the Noise
 */
inline void ADnote::ComputeVoiceNoise(
    int nvoice)
{
    for (int k = 0; k < unison_size[nvoice]; ++k)
    {
    ...
    float *outr)
{
    ...
    memset(bypassl, 0, SystemSettings::Instance().bufferbytes);
    memset(bypassr, 0, SystemSettings::Instance().bufferbytes);
    computecurrentparameters();

    for (unsigned nvoice = 0; nvoice < NUM_VOICES; ++nvoice)
    {
        if ((NoteVoicePar[nvoice].Enabled != ON) || (NoteVoicePar[nvoice].DelayTicks > 0))
            continue;
        ...
        memset(tmpwavel, 0, SystemSettings::Instance().bufferbytes);
        if (stereo)
            memset(tmpwaver, 0, SystemSettings::Instance().bufferbytes);
        for (int k = 0; k < unison_size[nvoice]; ++k)
        {
            float *tw = tmpwave_unison[k];
            if (stereo)
            {
                float stereo_pos = 0;
                if (unison_size[nvoice] > 1)
                    stereo_pos = k / static_cast<float>(unison_size[nvoice] - 1) * 2.0f - 1.0f;
                ...
                if (stereo_spread > 1.0f)
                {
                    ...
                    stereo_pos =
                        (2.0f - stereo_spread) * stereo_pos + (stereo_spread - 1.0f) * stereo_pos_1;
                }
                else
        ...
        {
            unsigned int rest = SystemSettings::Instance().buffersize;
            //test if the amplitude if raising and the difference is high
            if ((newam > oldam) && ((newam - oldam) > 0.25f))
            {
                rest = 10;
                if (rest > SystemSettings::Instance().buffersize)
                    rest = SystemSettings::Instance().buffersize;
                for (unsigned int i = 0; i < SystemSettings::Instance().buffersize - rest; ++i)
                    tmpwavel[i] *= oldam;
                if (stereo)
                    for (unsigned int i = 0; i < SystemSettings::Instance().buffersize - rest; ++i)
                        tmpwaver[i] *= oldam;
            ...
            for (unsigned int i = 0; i < rest; ++i)
            {
                ...
                tmpwavel[i + (SystemSettings::Instance().buffersize - rest)] *= amp;
                if (stereo)
                    tmpwaver[i + (SystemSettings::Instance().buffersize - rest)] *= amp;
            }
        }
        else
        {
        ...
        if ((stereo) && (NoteVoicePar[nvoice].VoiceFilterR != nullptr))
        ...
        if (NoteVoicePar[nvoice].AmpEnvelope != nullptr)
            ...
            {
                for (unsigned int i = 0; i < SystemSettings::Instance().buffersize; ++i)
                    tmpwavel[i] *= 1.0f - static_cast<float>(i) / SystemSettings::Instance().buffersize_f;
                if (stereo)
                    for (unsigned int i = 0; i < SystemSettings::Instance().buffersize; ++i)
                        tmpwaver[i] *= 1.0f - static_cast<float>(i) / SystemSettings::Instance().buffersize_f;
            }
        ...
        if (NoteVoicePar[nvoice].VoiceOut != nullptr)
        {
            if (stereo)
                for (unsigned int i = 0; i < SystemSettings::Instance().buffersize; ++i)
        ...
        { //no bypass
            if (stereo)
                for (unsigned int i = 0; i < SystemSettings::Instance().buffersize; ++i)
                { //stereo
                    outl[i] += tmpwavel[i] * NoteVoicePar[nvoice].Volume * NoteVoicePar[nvoice].Panning * 2.0f;
                    outr[i] += tmpwaver[i] * NoteVoicePar[nvoice].Volume * (1.0f - NoteVoicePar[nvoice].Panning) * 2.0f;
                }
            else
                for (unsigned int i = 0; i < SystemSettings::Instance().buffersize; ++i) //mono
        ...
        { //bypass the filter
            if (stereo)
                for (unsigned int i = 0; i < SystemSettings::Instance().buffersize; ++i)
                { //stereo
                    bypassl[i] += tmpwavel[i] * NoteVoicePar[nvoice].Volume * NoteVoicePar[nvoice].Panning * 2.0f;
                    bypassr[i] += tmpwaver[i] * NoteVoicePar[nvoice].Volume * (1.0f - NoteVoicePar[nvoice].Panning) * 2.0f;
                }
            else
                for (unsigned int i = 0; i < SystemSettings::Instance().buffersize; ++i) //mono
    ...
    { //set the right channel=left channel
        ...
        memcpy(bypassr, bypassl, SystemSettings::Instance().bufferbytes);
    }
    else
...
template <class T>
static inline void nullify(T &t)
{
    delete t;
    t = nullptr;
}
template <class T>
static inline void arrayNullify(T &t)
{
    delete[] t;
    t = nullptr;
}
...
void ADnote::Voice::kill()
{
    ...
    if (VoiceOut)
    {
        memset(VoiceOut, 0, SystemSettings::Instance().bufferbytes);
...
void ADnote::Global::initparameters(
