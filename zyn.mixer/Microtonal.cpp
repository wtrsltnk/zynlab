...
void Microtonal::Defaults()
{
    ...
    {
        octave[i].tuning = tmpoctave[i].tuning = powf(2, (i % octavesize + 1) / 12.0f);
    ...
    snprintf(reinterpret_cast<char *>(Pname), MICROTONAL_MAX_NAME_LEN, "12tET");
    snprintf(reinterpret_cast<char *>(Pcomment), MICROTONAL_MAX_NAME_LEN, "Equal Temperament 12 notes per octave");
...
float Microtonal::getnotefreq(int note, int keyshift) const
{
    ...
    {
        note = static_cast<int>(Pinvertupdowncenter) * 2 - note;
    }
    ...
    int scaleshift = (static_cast<int>(Pscaleshift) - 64 + static_cast<int>(octavesize) * 100) % octavesize;

    //compute the keyshift
    float rap_keyshift = 1.0f;
    if (keyshift != 0)
    {
        int kskey = (keyshift + static_cast<int>(octavesize) * 100) % octavesize;
        int ksoct = (keyshift + static_cast<int>(octavesize) * 100) / octavesize - 100;
    ...
    if (Pmappingenabled != 0)
    {
        ...
        int degoct = (note - static_cast<int>(Pmiddlenote + Pmapsize * 200)) / static_cast<int>(Pmapsize - 200);
        int degkey = (note - Pmiddlenote + static_cast<int>(Pmapsize * 100)) % Pmapsize;
    ...
    int ntkey = (nt + static_cast<int>(octavesize * 100)) % octavesize;
...
#define FMCREQ(x)                                                  \
    ...
    if (strcmp(reinterpret_cast<const char *>(this->Pname), reinterpret_cast<const char *>(micro.Pname)) != 0)
    {
        return true;
    }
    if (strcmp(reinterpret_cast<const char *>(this->Pcomment), reinterpret_cast<const char *>(micro.Pcomment)) != 0)
    {
        return true;
    }
...
int Microtonal::linetotunings(unsigned int nline, const char *line)
{
    ...
    switch (type)
    {
        case 1:
        {
            x1 = static_cast<int>(std::floor(x));
            tmp = std::fmod(x, 1.0f);
            x2 = static_cast<int>(std::floor(tmp * 1e6f));
