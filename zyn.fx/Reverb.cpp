...
Reverb::Reverb(bool insertion_, float *efxoutl_, float *efxoutr_)
    : Effect(insertion_, efxoutl_, efxoutr_, nullptr, 0),
      ...
      idelay(nullptr),
      lpf(nullptr),
      hpf(nullptr) // no filter
{
    for (int i = 0; i < REV_COMBS * 2; ++i)
    {
        comblen[i] = 800 + static_cast<int>(RND * 1400.0f);
        combk[i] = 0;
        lpcomb[i] = 0;
        combfb[i] = -0.97f;
        comb[i] = nullptr;
    ...
    {
        aplen[i] = 500 + static_cast<int>(RND * 500.0f);
        apk[i] = 0;
        ap[i] = nullptr;
    }
    SetPreset(Ppreset);
...
void Reverb::processmono(int ch, float *output, const float *inputbuf)
{
...
void Reverb::settime(unsigned char _Ptime)
{
    ...
    for (int i = 0; i < REV_COMBS * 2; ++i)
    {
        combfb[i] = -expf(static_cast<float>(comblen[i] / SystemSettings::Instance().samplerate_f * logf(0.001f) / t));
    }
...
void Reverb::setlohidamp(unsigned char _Plohidamp)
{
    ...
    else
    {
        ...
        if (Plohidamp > 64)
            lohidamptype = 2;
        float x = fabsf(static_cast<float>(Plohidamp - 64) / 64.1f);
        lohifb = x * x;
    }
}
...
void Reverb::setidelay(unsigned char _Pidelay)
{
    ...
    idelaylen = static_cast<int>(SystemSettings::Instance().samplerate_f * delay / 1000);
    if (idelaylen > 1)
    {
        idelayk = 0;
...
void Reverb::settype(unsigned char _Ptype)
{
    ...
    float samplerate_adjust = SystemSettings::Instance().samplerate_f / 44100.0f;
    float tmp;
    for (int i = 0; i < REV_COMBS * 2; ++i)
    {
        if (Ptype == 0)
        {
            tmp = 800.0f + static_cast<int>(RND * 1400.0f);
        }
        else
        {
    ...
    for (int i = 0; i < REV_APS * 2; ++i)
    {
        if (Ptype == 0)
        {
            tmp = 500 + static_cast<int>(RND * 500.0f);
        }
        else
        {
    ...
    { //bandwidth
        ...
        //be found
        bandwidth = new Unison(SystemSettings::Instance().buffersize / 4 + 1, 2.0f, SystemSettings::Instance().samplerate_f);
