...
int main(int /*argc*/, char * /*argv*/ [])
{
    ...
    AppDrummer app(window, mixer, &bankManager);

    glfwSetWindowSizeCallback(window, AppDrummer::ResizeCallback);
    glfwMakeContextCurrent(window);

    gladLoadGL();

    AppDrummer::ResizeCallback(window, 1024, 768);

    if (app.Setup())
    {
        glfwSetKeyCallback(window, AppDrummer::KeyActionCallback);
